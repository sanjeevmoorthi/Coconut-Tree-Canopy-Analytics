"""
Train a YOLOv11 segmentation model to detect and segment coconut tree crowns in drone imagery.

Usage:
    python scripts/train_seg.py
    python scripts/train_seg.py --epochs 100 --batch-size 16 --weights yolo11n-seg.pt
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train coconut tree segmentation model")
    parser.add_argument("--config", default="config/config_seg.yaml")
    parser.add_argument("--epochs", type=int, default=None)
    parser.add_argument("--batch-size", type=int, default=None)
    parser.add_argument("--img-size", type=int, default=None)
    parser.add_argument("--weights", default=None, help="Base segmentation model, e.g. yolo11n-seg.pt")
    ...
    if not dataset_yaml.exists():
        raise SystemExit(
            ...
            "Run first: python scripts/convert_dataset_to_seg.py"
        )
    ...
    print(f"Training segmentation model {weights} on {dataset_yaml}")
    ...
    print(f"Run inference: python scripts/predict_seg_track.py --weights {best_weights} --source <video_or_image_or_folder>")
